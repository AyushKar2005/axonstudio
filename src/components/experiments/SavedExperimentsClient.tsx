...
export default function SavedExperimentsClient() {
  ...
  useEffect(() => {
    fetchCloudRuns()
      .then((cloudRuns) => setRuns(Array.isArray(cloudRuns) ? cloudRuns : []))
      .catch(() => setRuns([]))
  ...
  const best = useMemo(
    () =>
      runs.reduce<SavedRun | null>((w, r) => {
        if (!w) return r;
        return (r.metrics?.testAccuracy ?? r.metrics?.accuracy ?? 0) >
          (w.metrics?.testAccuracy ?? w.metrics?.accuracy ?? 0)
          ? r
          : w;
      }, null),
    [runs],
  ...
  const clear = async () => {
    ...
    setRuns([]);
    try {
      await Promise.all(previous.map((run) => deleteCloudRun(run.id)));
    } catch {
      setRuns(previous);
    }
  };
  ...
  const exportAll = () => {
    ...
    const a = Object.assign(document.createElement("a"), {
      href: url,
      download: `axon-runs-${Date.now()}.json`,
    });
  ...
  return (
    <PageShell>
      ...
      >
        ...
        <h1 style={heroTitle}>Your cloud run history.</h1>
        <p style={heroCopy}>
          Saved experiments are stored through Axon&apos;s Supabase backend and scoped to this browser workspace.
          Compare model runs, review architecture choices, and export history whenever you need it.
        </p>
                      ...
                      >
                        <motion.div
                          ...
                          animate={{
                            width: `${Math.min(
                              100,
                              Math.round((run.metrics?.testAccuracy ?? run.metrics?.accuracy ?? 0) * 100),
                            )}%`,
                          }}
