...
import { ToastContainer, useToast } from "@/components/playground/Toast";
import {
  panelBg, pct,
  rightPanelTitleStyle, rightPanelValueStyle,
} from "@/components/playground/ui";

const emptyMetrics: TrainingMetrics = {
  accuracy: 0, trainAccuracy: 0, testAccuracy: 0,
  generalizationGap: 0, trainCount: 0, testCount: 0,
  meanConfidence: 0, positiveRatio: 0, updateNorm: 0,
  examplesPerSecond: 0, layerNorms: [], layerUpdateNorms: [],
  misclassifiedCount: 0,
};

// Map shareStatus strings → toast variant
function variantFor(status: string): "success" | "error" | "loading" | "info" {
  if (status.includes("…") || status.includes("ing"))        return "loading";
  if (status.includes("failed") || status.includes("Could")) return "error";
  if (status.includes("copied") || status.includes("Saved") ||
      status.includes("loaded") || status.includes("Downloaded")) return "success";
  return "info";
}

export function RightPanel({
  state, width, displayLoss, displayEpoch, lossHistory, weights,
  metrics, predictions, inspection, explainCards, savedRuns, customDataset,
  onSaveRun, onRemoveRun, onClearInspection,
  onExportJson, onExportScreenshot, onCopySummary,
  onShareRun, shareStatus,
}: {
  ...
  const { toasts, show, replace } = useToast();
  const loadingIdRef = React.useRef<number | null>(null);

  // Every time shareStatus changes, fire the right toast
  useEffect(() => {
    if (!shareStatus) return;
    const variant = variantFor(shareStatus);

    if (variant === "loading") {
      // Show a new loading toast and remember its id so we can replace it
      const id = show(shareStatus, "loading");
      loadingIdRef.current = id;
    } else if (loadingIdRef.current !== null) {
      // Replace the pending loading toast in-place
      replace(loadingIdRef.current, shareStatus, variant);
      loadingIdRef.current = null;
    } else {
      show(shareStatus, variant);
    }
  ...
  return (
    <>
      <aside style={{
        ...
        background: panelBg, borderLeft: "1px solid #1e1e26",
        padding: "16px 13px",
        ...
        <Section title="Loss curve" value={displayLoss === null ? undefined : displayLoss.toFixed(4)}>
          <div style={{ height: 80, borderRadius: 8, border: "1px solid #1e1e26", overflow: "hidden", background: "rgba(255,255,255,0.012)" }}>
        ...
        <Section title="Point inspector">
          <PointInspector inspection={inspection} predictions={predictions} onClear={onClearInspection} />
        </Section>
        <Divider />
        ...
        <Section title="Weight heatmap">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(8,1fr)", gap: 3 }}>
            {(weights.length ? weights.slice(0, 40) : Array(40).fill(0)).map((w, i) => {
              ...
              return (
                <div key={i} title={weights.length ? w.toFixed(4) : "0.0000"} style={{ aspectRatio: "1", borderRadius: 3, background: weights.length ? bg : "rgba(255,255,255,0.025)", transition: "background 0.3s" }} />
              );
            })}
          </div>
            ...
            {(m.layerUpdateNorms.length
              ...
              <div key={i}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                  ...
                  <span style={{ fontSize: 10, color: "#4a4a58", fontFamily: "'JetBrains Mono', monospace" }}>{n.toFixed(4)}</span>
                </div>
                <div style={{ height: 3, borderRadius: 999, background: "#141418", overflow: "hidden" }}>
                  <div style={{ width: `${Math.min(100, (n / maxLayer) * 100)}%`, height: "100%", background: "linear-gradient(90deg,#3b82f6,#a855f7)", opacity: n ? 0.8 : 0.14, transition: "width 0.3s ease" }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <Section title="Summary">
          <MetricRows rows={[
            ...
            ["Params",  String(state.layers.reduce((acc, l, i) => i === 0 ? acc : acc + l.neurons * (state.layers[i - 1]?.neurons ?? 2) + l.neurons, 0))],
      ...
      {/* Rendered outside <aside> so it never gets clipped by overflow:hidden */}
...
function MetricRows({ rows }: { rows: [string, string][] }) {
  return (
    <div style={{ display: "grid", gap: 6 }}>
      {rows.map(([k, v]) => (
        <div key={k} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, padding: "4px 0", borderBottom: "1px solid rgba(255,255,255,0.024)" }}>
          <span style={{ fontSize: 11, color: "#38383e" }}>{k}</span>
          <span style={{ fontSize: 11, color: "#8a8a94", fontFamily: "'JetBrains Mono', monospace", textAlign: "right", letterSpacing: "0.02em" }}>{v}</span>
        </div>
      ))}
    </div>
  );
...
// React import needed for useRef in the component
