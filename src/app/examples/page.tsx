...
function MiniPlot({ type }: { type: string }) {
  const points = useMemo(() => Array.from({ length: 44 }, (_, i) => {
    const t = i / 44;
    if (type === "circles") {
      const a = t * Math.PI * 8;
      const r = i % 2 ? 34 : 16;
      return { x: 50 + Math.cos(a) * r + Math.sin(i) * 4, y: 50 + Math.sin(a) * r + Math.cos(i) * 4, c: i % 2 };
    }
    if (type === "spiral") {
      const a = t * Math.PI * 5;
      const r = t * 44;
      return { x: 50 + Math.cos(a + (i % 2) * Math.PI) * r, y: 50 + Math.sin(a + (i % 2) * Math.PI) * r, c: i % 2 };
    }
    if (type === "xor") {
    ...
    return { x: 22 + (i % 2) * 55 + (i * 1.7 % 15), y: 24 + (i % 2) * 45 + (i * 2.1 % 16), c: i % 2 };
  }), [type]);
  ...
  return (
    <svg viewBox="0 0 100 100" style={{ width: "100%", height: 140, display: "block" }}>
      ...
      <path d="M0 54 C22 32, 36 70, 55 49 S78 33, 100 54" stroke="rgba(255,255,255,0.10)" fill="none" strokeWidth="0.8" />
      {points.map((p, i) => (
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            <p style={{ color: "#a78bfa", textTransform: "uppercase", letterSpacing: "0.16em", fontSize: 10, fontWeight: 850, marginBottom: 14 }}>
              Experiment gallery
            </p>
            <h1 style={{ fontSize: "clamp(28px, 5vw, 58px)", letterSpacing: "-0.07em", lineHeight: 0.96, marginBottom: 16 }}>
              Start from demos that reveal<br />how models learn.
            </h1>
            <p style={{ color: "#a1a1aa", lineHeight: 1.8, fontSize: "clamp(13px, 1.5vw, 15px)", maxWidth: 640 }}>
          ...
          >
            <p style={{ fontSize: 10, color: "#52525b", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 10 }}>
              Search examples
            </p>
            ...
            >
              <svg
                ...
                style={{ position: "absolute", left: 11, top: "50%", transform: "translateY(-50%)", color: "#52525b", pointerEvents: "none" }}
              >
                ...
                {query && (
                  <motion.button
                    ...
                    style={{
                      ...
                      color: "#71717a", cursor: "pointer", padding: "2px 6px", fontSize: 10,
                    }}
                  >
                    ✕
                  </motion.button>
                ...
                >
                  ...
                  <div style={{ padding: "0 18px 20px" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                      <p style={{ color: "#a78bfa", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.12em" }}>
                        {e.tag}
                      </p>
                      <span
                        style={{
                    ...
                    <h2 style={{ fontSize: "clamp(17px, 2vw, 21px)", letterSpacing: "-0.04em", marginBottom: 8 }}>
                      {e.name}
                    </h2>
                    <p style={{ color: "#a1a1aa", lineHeight: 1.7, fontSize: 13, minHeight: 60, marginBottom: 14 }}>
                      {e.why}
                    </p>
                    ...
                    >
                      {[
                        ...
                        <div key={label}>
                          <span style={{ color: "#52525b", fontSize: 9, textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: 2 }}>
                            {label}
                          </span>
